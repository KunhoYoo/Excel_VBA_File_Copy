...
Sub copy_Files_In_Folder()

    Dim strPath As String          '파일 가져올 폴더 경로를 넣을 변수
    Dim strTarget As String       '파일이 복사될 폴더 경로를 넣을 변수
    ...
    For i = 1 To 4                                             '2회 반복
        strExt = Choose(i, "*.xls*", "*.jpg", "*.PCM*", "*wav", "*wav*")       '파일 확장자를 선택
        
        fileName = Dir(strPath & strExt)           '(폴더내)파일을 변수에 넣음
    
        Do While fileName <> ""                        '이름이 없지 않다면, 즉, 엑셀파일이 존재하면
            
            For Each rngC In Range("B1", Cells(Rows.Count, "B").End(3)) 'A열 각 셀을 순환
            
                If fileName = rngC Then                 '파일이름과 셀의 이름이 일치하면
                        
                    On Error Resume Next              '에러 발생해도 다음코드 진행
                    If FileLen(strTarget & fileName) > 0 Then  '복사될 폴더에 동일파일 존재여부 확인
                            
                        If Err <> 0 Then                       '동일한 파일이 없을 경우(에러가 난 경우)
                            FileCopy strPath & fileName, strTarget & fileName '파일을 복사
                        Else                                         '동일한 파일이 존재할 경우
                            ...
                            '--------------------------------------------------------------------
                            msgYN = MsgBox(fileName & " 파일이 존재합니다. 덮어쓸까요?", 3, "파일존재")
                            
                            Select Case msgYN            '메시지박스 결과를 선택
                                Case vbYes:                    'Yes 선택 시
                                    Kill strTarget & fileName  '기존파일 삭제 후 복사
                                    FileCopy strPath & fileName, strTarget & fileName '파일을 복사
